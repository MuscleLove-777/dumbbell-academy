...
const App = {
    ...
    init() {
        ...
        if (localStorage.getItem('dumbbell-darkmode') === 'true') {
    ...
    loadProgress() {
        try {
            const saved = localStorage.getItem('dumbbell-progress');
            if (saved) this.progress = JSON.parse(saved);
            const savedQuiz = localStorage.getItem('dumbbell-quiz-results');
    ...
    saveProgress() {
        localStorage.setItem('dumbbell-progress', JSON.stringify(this.progress));
        localStorage.setItem('dumbbell-quiz-results', JSON.stringify(this.quizResults));
    },
    ...
    showDashboard() {
        ...
        var html = '<div class="fade-in">' +
            ...
            '<h2>ダンベルトレーニング Academy へようこそ</h2>' +
            '<p>ダンベルを使った効果的なトレーニング方法を基礎から実践まで学べるプラットフォームです。</p>' +
            '</div>' +
    ...
    toggleDarkMode() {
        ...
        if (isDark) {
            ...
            localStorage.setItem('dumbbell-darkmode', 'false');
        } else {
            document.documentElement.setAttribute('data-theme', 'dark');
            localStorage.setItem('dumbbell-darkmode', 'true');
        }
    },
    ...
    resetProgress() {
        if (confirm('全ての学習進捗をリセットしますか？この操作は元に戻せません。')) {
            this.progress = {};
            this.quizResults = {};
            localStorage.removeItem('dumbbell-progress');
            localStorage.removeItem('dumbbell-quiz-results');
            this.buildSidebar();
            this.updateGlobalProgress();
            this.showDashboard();
        }
